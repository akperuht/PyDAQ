...
class realTimeGraph(QtWidgets.QMainWindow):
    ...
    def init_UI(self, *args, **kwargs): 
        ...
        **kwargs : 
        ...
        self.datalabels = ["Channel " + str(i) for i in range(-1,16)]
        self.datalabels[0] = 'Time (s)'
        if 'labels' in kwargs:
    ...
    def createWidgets(self):
        ...
        self.measParamsLayout.addWidget(self.measParamsLabel,0,0,1,3)
        self.measParamsLayout.addWidget(self.startButton,1,2,1,1)
        ...
        self.paramsLayout.addWidget(self.toolmenuChannelsMeasuredLabel,*(6,0),1,1)
        self.paramsLayout.addWidget(self.toolbuttonChannelsMeasured,*(6,2),1,1)
        #self.paramsLayout.addWidget(self.selectFileButton,*(7,2),1,1) # Span 1 column wide
        
        # Add widgets to tempFrame
        self.tempLayout.addWidget(self.thermometerLabel, 0,0,1,2)
        self.tempLayout.addWidget(self.comboRtherm_multip_label,*(1,0))
        self.tempLayout.addWidget(self.comboRtherm_multip,*(1,1))
        self.tempLayout.addWidget(self.comboThermChLabel,*(2,0))
        self.tempLayout.addWidget(self.comboThermCh,*(2,1))
        self.tempLayout.addWidget(self.comboThermTypeLabel,3,0)
        self.tempLayout.addWidget(self.comboThermType,3,1)
        self.tempLayout.addWidget(self.tempLabel,4,0,1,2)
    ...
    def addControls(self):
        ...
        # Set up comboboxes
        self.setupComboXY()
        self.comboGv.setStyleSheet(self.style)
        self.comboGi.setStyleSheet(self.style)
        ...
        self.thermometerLabel.setStyleSheet('QLabel {color: black;}')
        self.thermometerLabel.setFont(QtGui.QFont('Arial', 12,weight=QtGui.QFont.Bold))
    ...
    def createChannelSelector(self, chtype):
        ...
        if chtype == 'plotted':
            ...
            self.toolmenuChannelsMeasured = QtWidgets.QMenu(self)
            self.toolmenuChannelsMeasured.setStyleSheet(self.bstyle_toolmenu)
            # Add different channels to toolbutton
            self.actionChsm = []
            self.measCh_checkboxes = []
            i=0
            for di in self.available_channels:
                self.measCh_checkboxes.append(QtWidgets.QCheckBox(di, self.toolmenuChannelsMeasured))
                self.actionChsm.append(QtWidgets.QWidgetAction(self.toolmenuChannelsMeasured))
                self.actionChsm[-1].setDefaultWidget(self.measCh_checkboxes[-1])
                self.toolmenuChannelsMeasured.addAction(self.actionChsm[-1])
                i+=1
            self.toolbuttonChannelsMeasured.setMenu(self.toolmenuChannelsMeasured)
            self.toolbuttonChannelsMeasured.setPopupMode(QtWidgets.QToolButton.InstantPopup)
    ...
    def channelMenuMeasuredClicked(self):
        '''
        ...
        i = 0
        for measchb in self.measCh_checkboxes:
        ...
        self.combo_list = [self.datalabels[0]]+[self.datalabels[i+1] for i in range(len(self.datalabels)) if i in self.selected_channels_measured_index]
        self.setupComboXY()
        ...
        self.actionChsp = []
        j=0
        for i in self.selected_channels_measured_index:
            self.actionChsp.append(self.toolmenuChannelsPlotted.addAction(self.datalabels[i+1]))
        ...
        try:
            ...
            while not self.queue.empty():
                ...
                if datacount > self.memory_limit:
                    for di in data_in[:self.Nchannel+1]:
                        self.data[j].pop(0)
                        self.data[j].append(di)
                        j+=1
                # If datalimit is not broken, continue adding data to the array
                else:
                    for di in data_in[:self.Nchannel+1]:
                        self.data[j].append(di)
                        j+=1
...
def main(q1,q2,qin):
    ...
    app.setStyle("fusion")
    main = realTimeGraph()
    main.init_UI(Nchannel=16,continuous=False,memory_limit=5000000)  
